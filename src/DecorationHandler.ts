...
export default class DecorationHandler {
    ...
    public getDecorationType(addAnimation?: boolean): vscode.TextEditorDecorationType {
        ...
        let decorationType: vscode.TextEditorDecorationType = vscode.window.createTextEditorDecorationType({
            ...
            after: addAnimation ? {
                contentText: '',
                textDecoration: 'none; position: absolute; z-index: -1px; top: 0; margin-left: -8px; z-index: -1; padding: 0 2px; width: 4px; height: calc(100% - 1px); border-radius: 2px; animation-name: var(--bounce-bracket-animation-name); animation-duration: var(--bounce-bracket-animation-duration); animation-delay: var(--bounce-bracket-animation-delay)',
            } : undefined,
        });
